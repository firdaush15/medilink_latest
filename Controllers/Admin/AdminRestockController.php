...
class AdminRestockController extends Controller
{
    ...
    public function approve(Request $request, $id)
    {
        ...
        // Notify the pharmacist who submitted the request
        // Fixed: use the pharmacist's user_id from the relationship, not a hardcoded value
        $pharmacistUserId = $restockRequest->requestedBy->user_id ?? null;

        if ($pharmacistUserId) {
            \App\Models\StaffAlert::create([
                'sender_id'      => auth()->id(),
                'sender_type'    => 'admin',
                'recipient_id'   => $pharmacistUserId,
                ...
                'alert_message'  => "Your restock request {$restockRequest->request_number} for "
                                  . "{$restockRequest->medicine->medicine_name} has been approved. "
                                  . "You may proceed with ordering.",
    ...
    public function reject(Request $request, $id)
    {
        ...
        // Notify the pharmacist who submitted the request
        // Fixed: dynamically resolve the pharmacist's user_id
        $pharmacistUserId = $restockRequest->requestedBy->user_id ?? null;
        ...
        if ($pharmacistUserId) {
            \App\Models\StaffAlert::create([
                ...
                'alert_message'  => "Your restock request {$restockRequest->request_number} has been rejected. "
