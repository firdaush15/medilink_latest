...
class AppointmentController extends Controller
{
     ...
     *
     * Alert logic:
     *  - Doctor: ALWAYS alerted — slot is now free.
     *  - All Receptionists: ALWAYS alerted — they manage the schedule.
     *  - Assigned Nurse: ONLY if patient had already physically checked in
     *    (arrived_at is set AND assigned_nurse_id is set). If the patient
     *    cancels before arriving there is no nurse assigned yet.
     *
     * Note: Receptionists CANNOT cancel appointments (only patients can via
     * the mobile app). So this endpoint is the single cancellation pathway.
     */
    public function cancelAppointment(Request $request)
    {
        ...
        try {
            ...
            // ── Alert 1: Doctor ──────────────────────────────────────────────
            // Always fire — the doctor's slot is now free.
            StaffAlert::create([
            ...
            // Always fire — receptionists manage the front-desk schedule.
            // They need to know the slot opened up and can update the queue.
            ...
            // A nurse is assigned only when the receptionist physically checks
            // the patient in (arrived_at is set). If the patient cancels before
            // arriving, arrived_at is null and no nurse has been assigned.
            if (
