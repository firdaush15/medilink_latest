...
class PharmacistStockReceiptController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // 3. Quality check: Ensure expiry is at least 12 months from now
        ...
        if (
            !$validated['packaging_intact']
            || !$validated['temperature_maintained']
            || !$validated['expiry_acceptable']
        ) {
        ...
        try {
            ...
            // Alert admins if there are quality issues — dynamically find all admins
            if ($qualityStatus !== 'Accepted') {
                ...
                foreach ($adminUsers as $adminUser) {
                    StaffAlert::create([
                        ...
                        'priority'       => 'High',
                        'alert_title'    => '⚠️ Stock Receipt Quality Issue',
                        'alert_message'  => "Receipt {$receipt->receipt_number} has quality concerns and is ON HOLD. "
                                          . "Review required before stock can be added to inventory.",
