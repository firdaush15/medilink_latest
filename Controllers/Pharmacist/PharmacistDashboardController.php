...
class PharmacistDashboardController extends Controller
{
    public function index()
    {
        ...
        // ========================================
        // INVENTORY STATISTICS - WITH BATCH TRACKING
        // ========================================
        ...
        // Expiring soon (within 180 days / 6 months) - checks batches
        $expiringCount = MedicineInventory::whereHas('batches', function($query) {
            $query->where('expiry_date', '<=', now()->addDays(180))
                  ->where('expiry_date', '>', now())
                  ->where('quantity', '>', 0);
        })->count();
        
        // Expired items - checks batches
        $expiredCount = MedicineInventory::whereHas('batches', function($query) {
