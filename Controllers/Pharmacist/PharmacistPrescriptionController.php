...
class PharmacistPrescriptionController extends Controller
{
    public function index(Request $request)
    {
        ...
        switch ($status) {
            case 'pending':   $query->where('verification_status', 'Pending'); break;
            case 'verified':  $query->where('verification_status', 'Verified'); break;
            case 'dispensed': $query->where('verification_status', 'Dispensed'); break;
            case 'rejected':  $query->where('verification_status', 'Rejected'); break;
        }
        ...
        if ($request->has('date')) {
            switch ($request->get('date')) {
                ...
                case 'this_month': $query->whereMonth('created_at', now()->month); break;
            }
        }
        ...
        return view('pharmacist.pharmacist_prescriptions', compact(
            'prescriptions', 'doctors', 'pendingCount', 'verifiedCount', 'dispensedCount'
        ));
    }
    ...
    public function show($id)
    {
        ...
        return view('pharmacist.pharmacist_prescriptionDetails', compact(
            'prescription', 'dispensing', 'allergyWarnings'
        ));
    }
    ...
    public function verify(Request $request, $id)
    {
        try {
            ...
            return redirect()->back()->with('error', 'Failed to verify prescription: ' . $e->getMessage());
        }
    }
    ...
    public function reject(Request $request, $id)
    {
        try {
            ...
            // Alert the prescribing doctor about the rejection
            ...
            if ($prescription && $prescription->doctor) {
                StaffAlert::create([
                    ...
                    'alert_type'      => 'Prescription Rejected',
                    'priority'        => 'High',
                    'alert_title'     => '❌ Prescription Rejected',
                    'alert_message'   => "Your prescription for patient "
                                      . ($prescription->patient->user->name ?? 'N/A')
                                      . " has been rejected. Reason: {$validated['rejection_reason']}",
                    'action_url'      => route('pharmacist.prescriptions.show', $id),
                ]);
            }
            ...
            return redirect()->back()->with('error', 'Failed to reject prescription: ' . $e->getMessage());
        }
    }
            ...
            try {
                ...
                foreach ($prescription->items as $item) {
                    ...
                    if ($medicine->quantity_in_stock < $item->quantity_prescribed) {
                        ...
                        return response()->json([
                            'success' => false,
                            'message' => "Insufficient stock for {$medicine->medicine_name}. "
                                       . "Available: {$medicine->quantity_in_stock}, Required: {$item->quantity_prescribed}"
                        ], 400);
                    }
                ...
                // Alert receptionist(s) that patient is ready for checkout
                $this->alertReceptionistForCheckout($dispensing, $prescription, $totalAmount);

                return response()->json([
                    'success' => true,
                    'message' => 'Prescription dispensed successfully. Total: RM ' . number_format($totalAmount, 2) .
                                 '. Patient should proceed to receptionist for checkout.',
    ...
    /**
     * Alert receptionist(s) that patient is ready for checkout after dispensing.
     *
     * Priority order:
     *  1. The receptionist who originally checked the patient in (checked_in_by).
     *  2. If none found, alert ALL receptionists as fallback.
     *
     * No hardcoded user IDs.
     */
    protected function alertReceptionistForCheckout($dispensing, $prescription, $totalAmount)
    {
        try {
            $patientName  = $dispensing->patient->user->name ?? 'Patient';
            $amountStr    = 'RM ' . number_format($totalAmount, 2);
            $appointment  = $prescription->appointment;

            $alertTitle   = '✅ Patient Ready for Checkout';
            $alertMessage = "Patient {$patientName}'s prescription has been dispensed ({$amountStr}). "
                          . "Please process payment and checkout.";

            // Try to get a specific route; fall back gracefully
            $actionUrl = null;
            try {
                $actionUrl = $appointment
                    ? route('receptionist.checkout.show', $appointment->appointment_id)
                    : route('receptionist.checkout');
            } catch (\Exception $e) {
                try {
                    $actionUrl = route('receptionist.checkout');
                } catch (\Exception $ex) {
                    // Route doesn't exist; leave null
                }
            }

            // Primary: alert the specific receptionist who checked this patient in
            $alertedCount = 0;
            if ($appointment && $appointment->checked_in_by) {
                $checkInReceptionist = User::where('id', $appointment->checked_in_by)
                    ->where('role', 'receptionist')
                    ->first();

                if ($checkInReceptionist) {
                    StaffAlert::create([
                        'sender_id'       => auth()->id(),
                        'sender_type'     => 'pharmacist',
                        'recipient_id'    => $checkInReceptionist->id,
                        'recipient_type'  => 'receptionist',
                        'patient_id'      => $dispensing->patient_id,
                        'prescription_id' => $prescription->prescription_id,
                        'alert_type'      => 'Ready for Checkout',
                        'priority'        => 'Normal',
            ...
            // Fallback: alert ALL receptionists if the specific one wasn't found
            if ($alertedCount === 0) {
            ...
            \Log::error('Failed to send checkout alert: ' . $e->getMessage());
        }
    }
}