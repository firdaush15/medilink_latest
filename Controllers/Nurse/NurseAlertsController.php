...
class NurseAlertsController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('nurse.nurse_alerts', compact(
            'alerts', 'filter', 'counts',
            'doctors', 'adminUsers', 'pharmacists', 'receptionists',
            'recentPatients'
        ));
    }
    ...
    /**
     * Generic send endpoint — nurse can send to doctor, admin, pharmacist, or receptionist.
     */
    public function send(Request $request)
    {
        $validated = $request->validate([
            'recipient_id'   => 'required|exists:users,id',
            'recipient_type' => 'required|in:doctor,admin,pharmacist,receptionist',
        ...
        $actionRouteMap = [
            'doctor'       => 'doctor.alerts.inbox',
            'admin'        => 'admin.alerts.index',
            'pharmacist'   => 'pharmacist.alerts.index',
            'receptionist' => 'receptionist.alerts.index',
        ];

        $actionUrl = null;
        try {
            $routeName = $actionRouteMap[$validated['recipient_type']] ?? null;
            if ($routeName) {
    ...
    /**
     * Convenience: send to doctor specifically.
     * Kept for backward-compat with any existing routes.
     */
    public function sendToDoctor(Request $request)
    {
        ...
        // Confirm target is actually a doctor
        $doctorUser = User::where('id', $validated['doctor_user_id'])
            ->where('role', 'doctor')
            ->firstOrFail();
    ...
    /**
     * Convenience: send to admin specifically.
     * Kept for backward-compat with any existing routes.
     */
