...
class NursePatientsController extends Controller
{
    ...
    public function __construct(NurseAssignmentService $assignmentService)
    {
    ...
    /**
     * Store vitals and fire all required alerts:
     *  1. Acknowledge the check-in alert for this nurse.
     *  2. Alert the doctor (critical or normal "patient ready").
     *  3. If not critical → auto-mark ready for doctor.
     */
    ...
    /**
     * Acknowledge the "Patient Checked In" alert that was sent to this nurse
     * when the receptionist checked the patient in.
     */
    ...
    /**
     * Alert the doctor:
     *  - Critical vitals  → 'Critical' priority, immediate attention required.
     *  - Normal vitals    → 'Normal' priority, patient is ready for consultation.
     *
     * This is the primary mechanism by which the doctor is notified that a
     * patient is ready. The alert appears in the doctor's alert inbox and
     * increments their navbar badge.
     */
    protected function createVitalsAlertForDoctor(Appointment $appointment, VitalRecord $vital, bool $isCritical)
    {
        ...
        if ($isCritical) {
            ...
            $message = "URGENT: Patient {$appointment->patient->user->name} "
                     . "(Queue #{$appointment->formatted_queue_number}) has CRITICAL vital signs:\n\n"
            ...
            $message = "Patient {$appointment->patient->user->name} "
                     . "(Queue #{$appointment->formatted_queue_number}) vitals recorded and verified. "
        ...
        Log::info(
            "Vitals alert sent to Dr. {$appointment->doctor->user->name} "
            . "for patient {$appointment->patient->user->name} "
    ...
    public function show($id)
    {
        ...
        return view('nurse.nurse_patientDetails', compact(
            'patient',
            'vitalRecords',
            'medicalRecords'
        ));
