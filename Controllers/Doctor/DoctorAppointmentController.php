    ...
    public function updatePatientPage($id)
    {
        ...
        if (!$appointment->isReadyForDoctor() && !$appointment->isWithDoctor() && !$appointment->isCompleted()) {
            return redirect()
                ...
                ->with('error', 'Cannot access patient details. Patient must complete check-in and vitals recording first.');
        }
        ...
        // ✅ FIXED: Fetch the list of specialists (excluding the current doctor)
        $specialists = Doctor::with('user')
            ->where('doctor_id', '!=', $doctor->doctor_id)
            ->get();

        // ✅ FIXED: Add 'specialists' to the compact() function
