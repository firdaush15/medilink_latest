...
class DoctorPrescriptionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!$request->has('allergy_override')) {
            ...
            // 🔴 CHANGED: Use with() instead of withErrors() for the data arrays
            // to prevent them from being converted to strings by MessageBag
            if (!empty($allergyWarnings['critical'])) {
                return redirect()
                    ->back()
                    ->withErrors(['allergy_error' => 'CRITICAL ALLERGY DETECTED: Prescription Blocked'])
                    ->with('critical_allergies', $allergyWarnings['critical']) // ✅ Passed as session data
                    ->withInput();
            }
            ...
            if (!empty($allergyWarnings['warnings'])) {
                return redirect()
                    ...
                    ->with('allergy_warnings', $allergyWarnings['warnings']) // ✅ Passed as session data
                    ->withInput();
            }
        }
