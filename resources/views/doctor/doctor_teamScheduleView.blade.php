    ...
    <div class="main-content">
        ...
        <div id="leave-tab" class="tab-content">
            <div class="leave-section">
                <h2 style="margin-bottom: 20px;">Leave Balance ({{ date('Y') }})</h2>

                <div class="leave-stats">
                    <!-- ✅ UPDATED: Using new structure from LeaveEntitlement model -->
                    <div class="stat-card">
                        <div class="stat-value">{{ $leaveBalance['annual']['remaining'] }}</div>
                        <div class="stat-label">Annual Leave Remaining</div>
                        <div style="font-size: 11px; color: #718096; margin-top: 5px;">
                            {{ $leaveBalance['annual']['used'] }} of {{ $leaveBalance['annual']['entitled'] }} used
                        </div>
                    </div>

                    <div class="stat-card">
                        <div class="stat-value">{{ $leaveBalance['sick']['remaining'] }}</div>
                        <div class="stat-label">Sick Leave Remaining</div>
                        <div style="font-size: 11px; color: #718096; margin-top: 5px;">
                            {{ $leaveBalance['sick']['used'] }} of {{ $leaveBalance['sick']['entitled'] }} used
                        </div>
                    </div>

                    <div class="stat-card">
                        <div class="stat-value">{{ $leaveBalance['emergency']['remaining'] }}</div>
                        <div class="stat-label">Emergency Leave Remaining</div>
                        <div style="font-size: 11px; color: #718096; margin-top: 5px;">
                            {{ $leaveBalance['emergency']['used'] }} of {{ $leaveBalance['emergency']['entitled'] }} used
                        </div>
                    </div>

                    <div class="stat-card">
                        <div class="stat-value">{{ $leaveBalance['total_used'] }}</div>
                ...
                <form action="{{ route('doctor.leave.apply') }}" method="POST">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        <div class="form-group">
                            ...
                            <select name="leave_type" required>
                                ...
                                <option value="sick" {{ old('leave_type') == 'sick' ? 'selected' : '' }}>
                                    Sick Leave ({{ $leaveBalance['sick']['remaining'] }} days remaining)
                                </option>
                                <option value="emergency" {{ old('leave_type') == 'emergency' ? 'selected' : '' }}>
                                    Emergency Leave ({{ $leaveBalance['emergency']['remaining'] }} days remaining)
                                </option>
                        ...
                        <div class="form-group">
                            ...
                            <select name="is_half_day">
                                ...
                                <option value="1" {{ old('is_half_day') == '1' ? 'selected' : '' }}>Morning Half</option>
                                <option value="2" {{ old('is_half_day') == '2' ? 'selected' : '' }}>Afternoon Half</option>
                            </select>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="reason" required placeholder="Provide reason for leave request...">{{ old('reason') }}</textarea>
                    </div>
