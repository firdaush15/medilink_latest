...
<head>
    ...
    <title>Settings — Doctor | MediLink</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card-head {
            ...
            padding-bottom: 18px; margin-bottom: 22px; border-bottom: 1px solid #f1f5f9;
        }
        ...
        .upload-label {
            display: inline-block; padding: 8px 16px; background: #1d4ed8;
            color: #fff; border-radius: 8px; font-size: 0.82rem; font-weight: 600; cursor: pointer;
        }
        .upload-label:hover { background: #1e40af; }
        ...
        .form-control:focus { border-color: #1d4ed8; box-shadow: 0 0 0 3px rgba(29,78,216,.08); }
        textarea.form-control { resize: vertical; min-height: 72px; }

        .btn {
            display: inline-flex; align-items: center; gap: 7px;
            padding: 10px 22px; border-radius: 9px;
            font-size: 0.86rem; font-weight: 600; font-family: 'Poppins', sans-serif;
            cursor: pointer; border: none; transition: all .2s;
        }
        .btn-primary   { background: #1d4ed8; color: #fff; }
        .btn-primary:hover { background: #1e40af; }
        .btn-secondary { background: #f1f5f9; color: #1d4ed8; border: 1.5px solid #bfdbfe; }
        ...
        .tips-list li::before { content: "• "; color: #1d4ed8; }
    ...
    <div class="content-area">
        ...
        <div class="page-header">
            <div>
                ...
                <p class="page-sub">Manage your profile, availability and account security</p>
            </div>
            <span class="role-chip">👨‍⚕️ Doctor</span>
        </div>
        ...
        <div class="settings-grid">
            ...
            <div class="card">
                <div class="card-head">
                    ...
                    <div>
                        ...
                        <p class="card-desc">Update your personal and clinical details</p>
                    </div>
                </div>

                <form action="{{ route('doctor.setting.profile') }}" method="POST" enctype="multipart/form-data">
                    @csrf

                    <div class="avatar-row">
                        @if(auth()->user()->doctor?->profile_photo)
                            <img src="{{ asset('storage/' . auth()->user()->doctor->profile_photo) }}" class="avatar-img" alt="Avatar">
                        @else
                            <div class="avatar-circle">{{ strtoupper(substr(auth()->user()->name, 0, 2)) }}</div>
                        @endif
                        <div>
                            <label class="upload-label" for="doc_photo">📷 Change Photo</label>
                            <input type="file" id="doc_photo" name="profile_photo" class="hidden" accept="image/*">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="phone_number" class="form-control"
                                value="{{ auth()->user()->doctor?->phone_number }}" placeholder="+60 1X-XXX XXXX">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Specialization</label>
                            <input type="text" name="specialization" class="form-control"
                                value="{{ auth()->user()->doctor?->specialization }}" placeholder="e.g. Cardiology">
                        </div>
                    </div>
                    <div class="form-group">
            ...
            <div class="card">
                ...
                <form action="{{ route('doctor.setting.password') }}" method="POST">
                    @csrf
                    <div class="form-group">
            ...
            <div class="card">
                <div class="card-head">
                    <span class="card-icon">🩺</span>
                    <div>
                        <p class="card-title">Professional Details</p>
                        <p class="card-desc">Your clinical info and availability status</p>
                    </div>
                </div>

                @php
                    $doctor = auth()->user()->doctor;
                    $avail  = $doctor?->availability_status ?? 'Available';
                @endphp

                <div class="info-grid">
                    <div class="info-tile">
                        <p class="info-tile-label">Specialization</p>
                        <p class="info-tile-value">{{ $doctor?->specialization ?? '—' }}</p>
                    </div>
                    <div class="info-tile">
                    ...
                    <div class="info-tile">
                        <p class="info-tile-label">Assigned Nurses</p>
                        <p class="info-tile-value">{{ $doctor?->assignedNurses()->count() ?? 0 }}</p>
                    </div>
                    <div class="info-tile">
                        <p class="info-tile-label">Role</p>
                        <span class="pill pill-blue">Doctor</span>
                    </div>
                </div>

                <form action="{{ route('doctor.setting.availability') }}" method="POST">
                    @csrf
                    <div class="avail-row">
                        <span class="avail-label">Update Availability:</span>
            ...
            <div class="card card-danger">
                ...
                <div class="danger-row">
                    <div>
                        ...
                        <p class="danger-hint">Permanently remove your account and all associated data</p>
                    </div>
