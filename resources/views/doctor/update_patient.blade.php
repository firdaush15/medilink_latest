...
<head>
  ...
  @vite(['resources/css/sidebar.css', 'resources/css/doctor/update_patient.css'])
  <style>
    /* ========================================
   ENCOUNTER TIMER STYLES - FIXED
   ======================================== */
    .encounter-timer-bar {
   ...
   ======================================== */
    ...
    /* ========================================
   FIXED: PREVENT DUPLICATE STICKY ELEMENTS
   ======================================== */
    .patient-header {
      background: white;
      border-radius: 12px;
   ...
   ======================================== */
    ...
    .alert ul li strong::before {
      display: none;
    }

    /* ICD-10 Code Badge */
    .alert ul li strong::after {
   ...
   ======================================== */
    ...
    .referral-section-info {
      flex: 1;
      min-width: 200px;
    }

    .referral-section-info h4 {
      margin: 0 0 8px 0;
      color: #1e40af;
      font-size: 16px;
    }

    .referral-section-info p {
      margin: 0;
      color: #6b7280;
      font-size: 14px;
    }
    ...
    /* ========================================
   PATIENT HISTORY TAB - IMPROVED LAYOUT
   ======================================== */
    .history-section {
      margin-bottom: 40px;
    }
...
<body>
  ...
  <div class="main">
    ...
    <div id="prescriptions" class="tab-content">
      ...
      <form action="{{ route('doctor.prescriptions.store') }}" method="POST" class="modern-form">
        ...
        {{-- ============================================================= --}}
        {{-- 🚨 CRITICAL ALLERGY ALERT (Blocks Submission)                 --}}
        {{-- ============================================================= --}}
        @if($errors->has('allergy_error'))
        <div style="background-color: #fee2e2; border: 2px solid #ef4444; color: #991b1b; padding: 15px; border-radius: 8px; margin-bottom: 20px; animation: slideIn 0.3s ease;">
          <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
            <svg width="24" height="24" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <h3 style="margin: 0; font-size: 16px; font-weight: bold;">CRITICAL ALLERGY ALERT</h3>
          </div>
          <p style="margin-bottom: 10px;">The following medications cannot be prescribed due to severe allergies:</p>
          <ul style="background: white; padding: 10px 10px 10px 30px; border-radius: 6px; border: 1px solid #fca5a5; margin: 0;">
            @foreach(session('critical_allergies', []) as $alert)
            <li style="margin-bottom: 5px;">
              <strong>{{ $alert['medicine'] }}</strong> conflicts with
              <span style="color: #dc2626; font-weight: bold;">{{ $alert['allergen'] }}</span>
              <span style="font-size: 11px; background: #fee2e2; padding: 2px 6px; border-radius: 4px; border: 1px solid #fecaca; margin-left: 5px;">{{ $alert['severity'] }}</span>
            </li>
            @endforeach
          </ul>
        </div>
        ...
        {{-- ============================================================= --}}
        {{-- ⚠️ ALLERGY WARNING (Allows Override)                          --}}
        {{-- ============================================================= --}}
        @if($errors->has('allergy_warning'))
        <div style="background-color: #fffbeb; border: 2px solid #f59e0b; color: #92400e; padding: 15px; border-radius: 8px; margin-bottom: 20px; animation: slideIn 0.3s ease;">
          <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
            <svg width="24" height="24" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <h3 style="margin: 0; font-size: 16px; font-weight: bold;">ALLERGY WARNING</h3>
          </div>
          <p>Please review the following potential interactions:</p>
          <ul style="margin-bottom: 15px; padding-left: 20px;">
            @foreach(session('allergy_warnings', []) as $alert)
            <li>
              <strong>{{ $alert['medicine'] }}</strong> may interact with {{ $alert['allergen'] }} ({{ $alert['severity'] }})
            </li>
            @endforeach
          </ul>

          <div style="padding-top: 10px; border-top: 1px solid #fcd34d;">
            <label style="display: flex; align-items: start; gap: 10px; cursor: pointer; user-select: none;">
              <input type="checkbox" name="allergy_override" value="1" style="margin-top: 4px; width: 16px; height: 16px;">
              <div>
                <strong style="color: #b45309;">Override Safety Check</strong>
                <div style="font-size: 13px; opacity: 0.9;">I confirm I have reviewed the risks and wish to proceed.</div>
              </div>
            </label>
            <input type="text" name="allergy_override_reason" placeholder="Reason for override (Required if checked)" style="width: 100%; margin-top: 10px; padding: 10px; border: 1px solid #d97706; border-radius: 6px; font-size: 14px;">
          </div>
        </div>
        @endif
        ...
        <div class="form-group">
          ...
          <textarea name="notes" rows="3" placeholder="General notes or instructions for the patient...">{{ old('notes') }}</textarea>
        </div>
        ...
        <div id="prescription-items">
          {{--
           If there are old inputs (e.g. after a validation error), we should ideally loop through them.
           However, to keep it simple and working with your current JS, we'll stick to the default item structure.
           The 'value' attributes below help retain data if validation fails.
        --}}
          <div class="prescription-item">
            ...
            <div class="form-group">
              ...
              <input type="text"
                ...
                value="{{ old('medicines.0.medicine_name') }}"
                ...
                id="medicine_name_0">
              <input type="hidden" name="medicines[0][medicine_id]" id="medicine_id_0" value="{{ old('medicines.0.medicine_id') }}">
              <input type="hidden" name="medicines[0][unit_price]" id="unit_price_0" value="{{ old('medicines.0.unit_price') }}">
            ...
            <div class="form-row">
              ...
              <div class="form-group">
                ...
                <select name="medicines[0][frequency]"
                  ...
                  <option value="1" {{ old('medicines.0.frequency') == '1' ? 'selected' : '' }}>Once daily (OD)</option>
                  <option value="2" {{ old('medicines.0.frequency') == '2' ? 'selected' : '' }}>Twice daily (BD)</option>
                  <option value="3" {{ old('medicines.0.frequency') == '3' ? 'selected' : '' }}>Three times daily (TDS)</option>
                  <option value="4" {{ old('medicines.0.frequency') == '4' ? 'selected' : '' }}>Four times daily (QDS)</option>
                  <option value="0.5" {{ old('medicines.0.frequency') == '0.5' ? 'selected' : '' }}>Every other day</option>
                  <option value="prn" {{ old('medicines.0.frequency') == 'prn' ? 'selected' : '' }}>As needed (PRN)</option>
                </select>
              </div>
              ...
              <div class="form-group">
                ...
                <input type="number"
                  ...
                  id="duration_0"
                  value="{{ old('medicines.0.duration') }}"
                  min="1"
                  max="365"
                  placeholder="e.g., 7"
                  required
            ...
            <div class="form-group">
              ...
              <input type="text"
                ...
                id="instructions_0"
                value="{{ old('medicines.0.instructions') }}"
            ...
            <input type="hidden" name="medicines[0][quantity_prescribed]" id="quantity_prescribed_0" value="{{ old('medicines.0.quantity_prescribed') }}">
          </div>
        </div>
    ...
    <div id="diagnosis" class="tab-content">
      ...
      <form action="{{ route('doctor.diagnoses.store') }}" method="POST" class="modern-form">
        ...
        <!-- Referral Section - New Clean Button -->
        <div class="form-group" style="background: linear-gradient(135deg, #f0f4ff 0%, #e0f2fe 100%); border: 2px dashed #3b82f6; border-radius: 12px; padding: 20px; margin-top: 20px;">
          <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 15px;">
            <div style="flex: 1; min-width: 200px;">
              <h4 style="margin: 0 0 8px 0; color: #1e40af; font-size: 16px;">
                🏥 Need to Refer Patient to a Specialist?
              </h4>
              <p style="margin: 0; color: #6b7280; font-size: 14px;">
                Book a follow-up appointment with an internal specialist doctor
              </p>
            </div>
            <button type="button"
              onclick="openReferralModal()"
              style="background: linear-gradient(135deg, #9333ea 0%, #7c3aed 100%); color: white; border: none; padding: 12px 24px; border-radius: 8px; font-weight: 600; display: flex; align-items: center; gap: 8px; cursor: pointer; transition: all 0.3s ease; white-space: nowrap; box-shadow: 0 4px 6px rgba(147, 51, 234, 0.2);">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
  ...
  <div id="referralModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true" style="z-index: 9999; display: none;">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0" style="display: flex; align-items: center; justify-content: center; min-height: 100vh; background: rgba(0,0,0,0.5); position: fixed; top: 0; left: 0; right: 0; bottom: 0;">

      <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" style="background: white; padding: 20px; border-radius: 12px; max-width: 550px; width: 100%; max-height: 90vh; overflow-y: auto;">

        <form action="{{ route('doctor.appointments.referral') }}" method="POST">
          @csrf
          <input type="hidden" name="patient_id" value="{{ $appointment->patient_id }}">
          <input type="hidden" name="referring_doctor_id" value="{{ $doctor->doctor_id }}">

          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="sm:flex sm:items-start" style="display: flex; gap: 15px;">
              <div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-purple-100 sm:mx-0 sm:h-10 sm:w-10" style="background: #f3e8ff; padding: 10px; border-radius: 50%; min-width: 50px; height: 50px;">
                <span style="font-size: 24px;">📅</span>
              </div>
              <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title" style="font-size: 20px; font-weight: 600; margin-bottom: 10px; color: #1f2937;">
                  Book Specialist Appointment
                </h3>
                <p style="font-size: 14px; color: #6b7280; margin-bottom: 20px;">
                  Refer <strong>{{ $appointment->patient->user->name }}</strong> to an internal specialist
                </p>
                ...
                <div class="mt-4 space-y-4">
                  ...
                  <div style="margin-bottom: 15px;">
                    <label class="block text-sm font-medium text-gray-700" style="display: block; margin-bottom: 5px; font-weight: 600;">
                      Select Specialist <span style="color: #dc3545;">*</span>
                    </label>
                    <select name="specialist_id"
                      ...
                      <option value="{{ $specialist->doctor_id }}">
                  ...
                  <div style="margin-bottom: 15px;">
                    <label class="block text-sm font-medium text-gray-700" style="display: block; margin-bottom: 5px; font-weight: 600;">
                      Appointment Date <span style="color: #dc3545;">*</span>
                    </label>
                    <input type="date"
                  ...
                  <div style="margin-bottom: 15px;">
                    <label class="block text-sm font-medium text-gray-700" style="display: block; margin-bottom: 5px; font-weight: 600;">
                      Appointment Time <span style="color: #dc3545;">*</span>
                    </label>
                    <input type="time"
                  ...
                  <div style="margin-bottom: 15px;">
                    <label class="block text-sm font-medium text-gray-700" style="display: block; margin-bottom: 5px; font-weight: 600;">
                      Referral Reason / Notes <span style="color: #dc3545;">*</span>
                    </label>
                    <textarea name="reason"
                      rows="3"
                      required
                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
                      style="width: 100%; padding: 10px; border: 2px solid #e5e7eb; border-radius: 6px; font-size: 14px; resize: vertical;"
                      placeholder="e.g., Needs cardiac evaluation for irregular heart rhythm detected during examination..."></textarea>
                    <small style="color: #6b7280; font-size: 12px;">
                      Provide context for the specialist about why this referral is needed
                    </small>
                  </div>
          ...
          <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse" style="display: flex; justify-content: flex-end; gap: 10px; margin-top: 20px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
            <button type="submit"
              ...
              style="padding: 10px 20px; background: #9333ea; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: 600; transition: all 0.3s ease;">
              ✓ Confirm Booking
            </button>
            <button type="button"
              onclick="closeReferralModal()"
              class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
              style="padding: 10px 20px; border: 1px solid #d1d5db; background: white; border-radius: 6px; cursor: pointer; font-weight: 600;">
              Cancel
            </button>
          </div>
        </form>
  ...
  `;
    ...
    document.addEventListener('click', function(e) {
      if (!e.target.closest('.form-group')) {
        // FIX: Exclude the static diagnosisSuggestions element from removal
        document.querySelectorAll('.medicine-suggestions').forEach(s => {
          if (s.id !== 'diagnosisSuggestions') {
            s.remove();
          }
        });
      }
    ...
    // ========================================
    // AUTO-OPEN PRESCRIPTION TAB ON ERROR (FIXED UI)
    // ========================================
    document.addEventListener('DOMContentLoaded', function() {
      @if($errors -> has('allergy_error') || $errors -> has('allergy_warning') || $errors -> has('medicines.*'))

      // 1. Hide all tabs first
      document.querySelectorAll('.tab-content').forEach(el => el.classList.remove('active'));
      document.querySelectorAll('.tab-btn').forEach(el => el.classList.remove('active'));

      // 2. Show Prescriptions Content
      const prescriptionTab = document.getElementById('prescriptions');
      if (prescriptionTab) {
        prescriptionTab.classList.add('active');

        // Scroll to the error message
        prescriptionTab.scrollIntoView({
          behavior: 'smooth',
          block: 'start'
        });
      }

      // 3. Highlight the Prescriptions Button (The Blue/Purple Gradient)
      // We search for the button that has the onclick="openTab('prescriptions')" attribute
      const prescriptionBtn = document.querySelector('button[onclick="openTab(\'prescriptions\')"]');
      if (prescriptionBtn) {
