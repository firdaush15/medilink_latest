{{-- resources/views/receptionist/receptionist_messages.blade.php --}}
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  @vite(['resources/css/sidebar.css', 'resources/css/receptionist/receptionist_messages.css'])
</head>
...
@endif
        ...
        <div class="quick-templates">
          ...
          <button class="template-btn" onclick="insertTemplate('Your appointment is confirmed for the scheduled time.')">✅ Appt Confirmed</button>
          <button class="template-btn" onclick="insertTemplate('Please bring your IC and insurance card to your appointment.')">📋 Reminder</button>
          <button class="template-btn" onclick="insertTemplate('Your appointment has been rescheduled. Please call us to confirm the new time.')">📅 Rescheduled</button>
          <button class="template-btn" onclick="insertTemplate('Patient has arrived and is waiting at the front desk.')">🧑 Patient Arrived</button>
        </div>
      ...
      <form id="newMessageForm" onsubmit="sendNewMessage(event)">
        ...
        <div class="form-group">
          ...
          <input type="text" name="subject" id="newMsgSubject"
                 placeholder="e.g., Appointment Query, Patient Check-in" required>
        </div>
  ...
  <script>
    ...
    function sendMessage(event) {
      ...
      fetch('{{ route("receptionist.messages.send") }}', {
          method: 'POST', body: fd,
    ...
    function sendNewMessage(event) {
      ...
      fetch('{{ route("receptionist.messages.create") }}', {
          method: 'POST', body: fd,
          headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' }
        })
        .then(r => { if (!r.ok) return r.json().then(e => { throw e; }); return r.json(); })
        .then(data => {
          if (data.success || data.conversation_id) {
            ...
            window.location.href = `{{ route('receptionist.messages') }}?conversation_id=${data.conversation_id}&type=${currentType}`;
          } else { alert(data.message || 'Failed to send.'); }
        })
        .catch(e => { console.error(e); alert(e.message || 'Failed to send.'); })
    ...
    function selectConversation(id) {
      const currentType = '{{ $type }}';
      window.location.href = `{{ route('receptionist.messages') }}?conversation_id=${id}&type=${currentType}`;
    }

    function toggleStar(id) {
      fetch(`/receptionist/messages/${id}/toggle-star`, {
          method: 'POST',
  ...
  <script>
(function () {
    const INBOX_POLL_URL      = '{{ route("receptionist.messages.inbox-poll") }}';
...
@if($selectedConversation)
  <script>
    window.POLL_URL        = '{{ route("receptionist.messages.poll", $selectedConversation->conversation_id) }}';
