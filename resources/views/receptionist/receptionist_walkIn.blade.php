...
<head>
    ...
    @vite(['resources/css/sidebar.css', 'resources/css/receptionist/receptionist_appointmentCreate.css'])
    <style>
        .urgency-selector {
            display: flex;
            gap: 15px;
...
<div id="new-patient-section" style="display: none;">
                            <div class="alert-box" style="background: #e3f2fd; padding: 12px; border-radius: 6px; margin-bottom: 15px; border-left: 4px solid #2196F3;">
                                <strong>📝 Quick Registration Mode</strong>
                                <p style="margin: 5px 0 0; font-size: 0.9em;">
                                    Enter IC Number to auto-fill details. Email is optional (leave blank for walk-in only).
                                </p>
                            </div>
                            
                            <div class="form-group">
                                <label for="new_patient_ic">IC Number <span class="required">*</span></label>
                                <input type="text" id="new_patient_ic" name="new_patient_ic" 
                                       placeholder="990101-01-1234" maxlength="14"
                                       oninput="formatIC(this)">
                                <small class="field-hint">Used as the unique ID for future app login</small>
                            </div>
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group">
                                    <label for="new_patient_email">Email (Optional)</label>
                                    <input type="email" id="new_patient_email" name="new_patient_email" placeholder="Only if patient wants app access now">
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        // Update toggle logic to make new fields required
        function togglePatientFields() {
            const type = document.getElementById('patient_type').value;
            document.getElementById('existing-patient-section').style.display = type === 'existing' ? 'block' : 'none';
            document.getElementById('new-patient-section').style.display = type === 'new' ? 'block' : 'none';
            ...
            document.getElementById('patient_id').required = isExisting;
            
            document.getElementById('new_patient_ic').required = isNew;
            document.getElementById('new_patient_name').required = isNew;
            document.getElementById('new_patient_phone').required = isNew;
