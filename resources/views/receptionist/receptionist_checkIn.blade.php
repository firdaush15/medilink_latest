...
<head>
    ...
    @vite(['resources/css/sidebar.css', 'resources/css/receptionist/receptionist_checkIn.css'])
    <style>
        .queue-badge {
...
<body>
   ...
   ======================================== */
    ...
    <script>
        // ========================================
// QUICK CHECK-IN SEARCH - FILTER APPOINTMENTS
// ========================================
...
function performSearch() {
    ...
    appointmentCards.forEach(card => {
        ...
        const phoneNumber = card.querySelector('.patient-details')?.textContent.toLowerCase() || '';
        ...
        if (matches) {
            ...
            card.style.animation = 'highlightCard 0.5s ease-in-out';
            visibleCount++;
        } else {
    ...
    if (visibleCount === 0) {
        ...
        noResultsMessage.innerHTML = `
            <div style="text-align: center; padding: 40px; color: #666;">
                ...
                <p style="margin: 0;">No appointments match "${query}"</p>
                <button onclick="clearSearch()" style="margin-top: 15px; padding: 8px 20px; background: #667eea; color: white; border: none; border-radius: 5px; cursor: pointer;">
                    Clear Search
                </button>
            </div>
        `;
        appointmentsGrid.appendChild(noResultsMessage);
    }
    
    updateSearchResultsCount(visibleCount, query);
    updateTabCounts(); // ✅ ADD THIS LINE
}
...
function updateSearchResultsCount(count, query) {
    ...
    const originalText = sectionHeader.getAttribute('data-original-text') || sectionHeader.textContent;
    
    if (!sectionHeader.hasAttribute('data-original-text')) {
        sectionHeader.setAttribute('data-original-text', originalText);
    }
    
    sectionHeader.innerHTML = `
        ${originalText}
        <span style="color: #667eea; font-weight: normal; font-size: 0.9em; margin-left: 10px;">
...
function resetSectionHeader() {
    const sectionHeader = document.querySelector('.section-header h2');
    const originalText = sectionHeader.getAttribute('data-original-text');
...
function updateTabCounts() {
    ...
    const counts = {
        ...
        pending: visibleCards.filter(card => card.dataset.status === 'pending').length,
        'checked-in': visibleCards.filter(card => card.dataset.status === 'checked-in').length,
        completed: visibleCards.filter(card => card.dataset.status === 'completed').length
    };
...
// Filter tabs functionality (updated to work with search)
