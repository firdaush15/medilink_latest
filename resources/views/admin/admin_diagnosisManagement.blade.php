...
<body>
    ...
    <div class="container">
        ...
        <div style="background: #fee2e2; color: #991b1b; padding: 15px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #fecaca;">
            <strong>⚠️ Whoops! Something went wrong:</strong>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <div class="checkbox-group">
                            <label>
                                <input type="checkbox" name="is_infectious" id="isInfectious" value="1">
                                Infectious
                            </label>
                            <label>
                                <input type="checkbox" name="is_chronic" id="isChronic" value="1">
                                Chronic
                            </label>
                            <label>
                                <input type="checkbox" name="requires_followup" id="requiresFollowup" value="1">
                                Requires Follow-up
                            </label>
                        </div>
                    </div>
    ...
    <script>
        // Check for errors on page load and reopen modal if necessary
        document.addEventListener('DOMContentLoaded', function() {
            @if($errors -> any())
            openAddModal();

            // Optional: Restore previous input values so the user doesn't have to retype
            document.getElementById('icd10Code').value = "{{ old('icd10_code') }}";
            document.getElementById('diagnosisName').value = "{{ old('diagnosis_name') }}";
            document.getElementById('category').value = "{{ old('category') }}";
            document.getElementById('severity').value = "{{ old('severity') }}";
            document.getElementById('description').value = "{{ old('description') }}";
            document.getElementById('recoveryDays').value = "{{ old('typical_recovery_days') }}";

            // Restore checkboxes
            document.getElementById('isInfectious').checked = @json(old('is_infectious') == 'on');
            document.getElementById('isChronic').checked = @json(old('is_chronic') == 'on');
            document.getElementById('requiresFollowup').checked = @json(old('requires_followup') == 'on');
            @endif
        });
        ...
        function openAddModal() {
            ...
            // RESET THE FORM ACTION URL (Add this line)
            document.getElementById('diagnosisForm').action = "{{ route('admin.diagnoses.store') }}";
